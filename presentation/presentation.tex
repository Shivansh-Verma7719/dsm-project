...
\begin{frame}{EDA: Who Participates?}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Income Gradient}
        \centering
        {\large\bfseries 10.7\% $\to$ 47.6\%}\\[0.3em]
        \small Q1 ($<$Rs.\,12,500) to Q5 ($>$Rs.\,35,000): a \textbf{4.5$\times$} spread.
        Participation nearly doubles with each income step.
      \end{block}
      \vspace{0.4em}
      \begin{block}{Urban Premium}
        \centering
        {\large\bfseries 28.8\% vs 17.6\%}\\[0.3em]
        \small Urban households are \textbf{1.6$\times$} more likely to participate.
        The gap widens further in lower income brackets.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Gender Gap}
        \centering
        {\large\bfseries 30.8\% vs 15.8\%}\\[0.3em]
        \small Male participation is nearly \textbf{2$\times$} female.
        The gap persists across every income level and zone.
      \end{block}
      \vspace{0.4em}
      \begin{block}{Generational Peak}
        \centering
        {\large\bfseries Millennials: 28.1\%}\\[0.3em]
        \small Ahead of Gen Z (24.3\%) and Gen X (22.9\%).
        Digital access appears to drive uptake more than experience.
      \end{block}
    \end{column}
  \end{columns}
  \vspace{0.4em}
  \small All four effects survive joint multivariate controls $\to$ Finding 1.
\end{frame}
...
\begin{frame}{EDA: Awareness Gap and Holdings}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \textbf{Awareness (all 109,430 respondents):}\\[0.6em]
      \begin{tabular}{lr}
        Fixed Deposits    & \textbf{99.1\%} \\[0.3em]
        Mutual Funds/ETFs & 67.0\%          \\[0.3em]
        Direct Equity     & 62.0\%          \\[0.3em]
        Derivatives       & \alert{17.9\%}  \\
      \end{tabular}
    \end{column}
    \begin{column}{0.46\textwidth}
      \textbf{Top holdings among investors:}\\[0.6em]
      \begin{tabular}{lr}
        Mutual Funds/ETFs & 66.5\% \\[0.3em]
        Fixed Deposits    & 57.2\% \\[0.3em]
        Direct Equity     & 49.3\% \\
      \end{tabular}
    \end{column}
  \end{columns}
  \vspace{0.4em}
  MFs and FDs co-dominate: investors layer market products on top of savings rather than replacing them.
  \vspace{0.3em}
  \begin{block}{The Awareness-Participation Gap}
    \small 67\% of households know mutual funds exist, yet only 25.5\% invest in any security.
    Awareness is necessary but not sufficient: trust, knowledge depth, and access remain barriers.
  \end{block}
\end{frame}
...
\begin{frame}{Finding 2: Three Archetypes of Non-Participation}
  K-means clustering on 18 self-reported barriers identifies three distinct profiles:
  \vspace{0.5em}
  \begin{block}{Trust-Deficient (46.3\%)}
    Urban, higher-income households who \emph{could} invest but choose not to.
    Top barriers: lack of trust in institutions and regulatory concerns.
    Requires accountability measures: grievance redressal, broker transparency.
  \end{block}
  \vspace{0.3em}
  \begin{block}{Knowledge-Gated (29.0\%)}
    Rural, lowest income quartile. Top barrier: ``I don't know how to start.''
    Needs geographically targeted literacy programs in local languages.
  \end{block}
  \vspace{0.3em}
  \begin{block}{Fear-Driven (24.8\%)}
    Spread across all demographics. Top barrier: fear of losing money.
    Responds to low-risk entry points that reduce perceived downside.
  \end{block}
  \vspace{0.4em}
...
\begin{frame}{Finding 4: Investor Education Programme Impact}
  \textbf{Only 2,768 investors (9.9\%)} cite SEBI or industry IEP as a
  top information source. Yet the difference in behaviour is striking:
  \vspace{0.6em}
  \begin{center}
    \begin{tabular}{lcc}
      \toprule
      Metric & IEP & Non-IEP \\
      \midrule
      Knowledge score (out of 9) & \textbf{5.30} & 3.75 \\
      Products aware             & 10.23 & 10.23 \\
      Goal-based motivation      & \textbf{30.2\%} & 22.8\% \\
      Quick-gains motivation     & 21.7\% & 23.3\% \\
      Holds Fixed Deposits       & 49.3\% & \textbf{55.1\%} \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{0.3em}
  \small The 41\% knowledge advantage is in \emph{depth}, not breadth: both groups
  are aware of the same number of products. IEP also reshapes \emph{how} investors
  participate: quick-gains motivation falls while goal-based motivation rises,
  a shift from speculative to intentional investing. The lower FD rate reflects this:
  educated investors substitute FDs for market-linked products. Current reach: \alert{9.9\%}.
\end{frame}
...
\begin{frame}{Policy Recommendations}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{A. Prioritise Product Awareness}\\
      \small Awareness predicts participation more than income.
      Campaigns should mention MFs, ETFs, SIPs by name.\\[0.5em]

      \normalsize\textbf{B. Tailor outreach by non-investor type}\\
      \small Trust-Deficient: institutional accountability.\\
      Knowledge-Gated: vernacular rural delivery.\\
      Fear-Driven: low-risk entry points.\\[0.5em]

      \normalsize\textbf{C. Expand investor education}\\
      \small SEBI's programs reach only 9.9\% of investors.
      Scale via workplaces and Jan Dhan accounts.
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{D. Close the gender gap}\\
      \small Women invest at half the rate of men even after
      controlling for income. Women-only groups and lower
      minimum amounts would help.\\[0.5em]

      \normalsize\textbf{E. Regulate financial influencers}\\
      \small Social media advice is linked to speculative
      behaviour. Require qualification disclosure and
      ban unverified return claims.\\[0.5em]

      \normalsize\textbf{F. Test knowledge before derivatives}\\
      \small Many investors overestimate their understanding
      and end up in high-risk products. Replace
      self-declaration with a short knowledge test.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Implementation: Stockbroker Sales AI}
  \begin{figure}
    \centering
    \includegraphics[width=\linewidth,height=0.82\textheight,keepaspectratio]{../figures/ai_layer.png}
  \end{figure}
\end{frame}
