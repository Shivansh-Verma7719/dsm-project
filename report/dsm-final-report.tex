...
To operationalize our findings, we tasked Random Forest classifiers to predict household behavior based strictly on basic demographic and risk-tolerance metrics. Can we forecast what a household will do before they even open a brokerage account?

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{../figures/fig7_predictive_importance.pdf}}
\caption{Feature Importance across the three Predictive Models. Education and Income dominate the primary participation decision.}
\label{fig:predictive_importance}
\end{figure}

As shown in Fig. \ref{fig:predictive_importance}, the models yielded illuminating results regarding the limits of demographic forecasting:
\begin{enumerate}
    \item \textbf{Model 1: Predicting Participation (AUC = 0.76)}: Basic metrics are highly effective at predicting \textit{whether} a household will participate in the market. Education and Income are the dominant features, confirming our logistic regression findings.
    \item \textbf{Model 2: Predicting Securities Choice (AUC = 0.60)}: Predicting \textit{how} they participate (choosing Equities vs. sticking to traditional FDs) is significantly harder. Once the wealth barrier is crossed, demographics lose their predictive power, and unobserved variables (like Finfluencer exposure) likely take over.
    \item \textbf{Model 3: Predicting Investment Horizon (AUC = 0.58)}: Forecasting \textit{how long} they will hold equity based on demographics is nearly indistinguishable from random chance.
\end{enumerate}
These predictive limits validate our earlier behavioral findings: while structural socioeconomic factors gatekeep entry into the market, post-entry behavior is overwhelmingly dictated by psychological factors, overconfidence, and modern social media dynamics.
