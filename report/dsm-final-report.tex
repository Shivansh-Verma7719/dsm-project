...
Among the 27,882 active investors, 2,768 (9.9\% of all investors; 28.3\% of the SS\_B3 information-source sub-module respondents) cite SEBI or industry Investor Education Programmes as one of their top information sources. As shown in Fig.~\ref{fig:iep_analysis}, IEP-exposed investors score markedly higher on the nine-item objective financial literacy quiz (mean score 5.30 vs.\ 3.75 for non-IEP investors, a 41\% gap), while product awareness is virtually identical across both groups (10.23 instruments known in each). This dissociation suggests that IEP exposure deepens conceptual understanding without broadening awareness: it converts existing knowledge into comprehension rather than expanding the universe of products a household has heard of.

The motivational profile of IEP-exposed investors also diverges sharply: 24.2\% cite long-term wealth growth as a primary motive versus 6.5\% of non-IEP investors, while quick-gains orientation is proportionally similar (21.7\% vs.\ 6.5\%), indicating that IEP steers investors toward patient capital formation. Taken together, these findings support IEP as a meaningful financial literacy lever, not merely an awareness campaign, with downstream effects on investment philosophy.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{../figures/fig8_iep_analysis.pdf}}
\caption{IEP-exposed vs.\ non-IEP investors. IEP exposure is associated with a 41\% higher financial literacy score but no difference in product awareness, and a stronger orientation toward long-term growth motives.}
...
\subsection{Goal--Portfolio Alignment: The Intent--Action Gap}
Beyond demographics and information sources, we test whether household portfolios are internally consistent with their stated primary financial goals. As shown in Fig.~\ref{fig:goal_alignment_heatmap}, households citing long-term objectives like \textit{Retirement} (72.4\%) or \textit{Children's Education} (77.2\%) show strong alignment, with high penetration in Mutual Funds and provident funds (PPF/EPF). 

However, we identify a stark ``Say--Do'' paradox among speculative entrants. While households citing \textit{Daily Trading} as their primary goal theoretically ought to hold high-risk, high-velocity instruments, we find an internal consistency score of only 7.3\% for this cohort (Fig.~\ref{fig:consistency_score}). 

This suggests a critical distinction between \textit{stated intent} and \textit{behavioral catalyst}. While many households adopt the identity of a ``trader'' as an aspirational goal, the actual adoption of speculative instruments like F\&O and Crypto is not driven by this formal objective. Instead, as demonstrated in our Section~\ref{sec:results} analysis of the Dunning-Kruger effect, risky asset penetration is overwhelmingly dictated by the \textbf{psychological trait of overconfidence} rather than the \textbf{stated goal of trading}. In essence, thousands of households \textit{want} to trade (Identity), but only the overconfident actually \textit{do} (Action), leaving a vast majority of self-identified traders holding traditional, misaligned portfolios.
