...
Table~\ref{tab:logit} presents the full logistic regression results ($N = 97{,}626$; McFadden pseudo-$R^2 = 0.208$, which falls in the ``excellent fit'' range of 0.20--0.40 on McFadden's scale and is not directly comparable to OLS $R^2$). Nine of eleven predictors are statistically significant; crucially, Gen~X (vs.\ Gen~Z reference) and West zone (vs.\ East reference) are not, with ORs of 1.015 and 1.004 respectively, providing confidence that the model is not returning spurious significance driven purely by sample size.
...
\midrule
\multicolumn{5}{l}{\textit{N} = 97,626 \quad McFadden pseudo-$R^2$ = 0.208 (excellent fit: 0.20--0.40) \quad All predictors standardised} \\
